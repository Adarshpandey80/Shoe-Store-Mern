...
const cartSlice = createSlice({
    ...
    reducers: {
        ...
        addWatchlistProd(state, action) {
            const exists = state.watchCart.find(
                item => item._id === action.payload._id
            );
            // ✅ Already in wishlist → do nothing
            if (exists) return;
            // ✅ Add once
            state.watchCart.push(action.payload);
        },
        removeWatchlistProd(state, action) {
            state.watchCart = state.watchCart.filter(
...
export const { addToCart, addWatchlistProd, removeWatchlistProd, decreaseQty, increaseQty, removeItem, buyNow, clearBuyNow, checkOutFromCart, clearMyCart } = cartSlice.actions;
